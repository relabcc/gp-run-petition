...
class DisableScroll {
  ...
  on(element, options) {
    ...
    /* istanbul ignore else */
    if (disableScroll) {
      this.lockToScrollPos = [scroll.getScrollX(), scroll.getScrollY()];
  ...
  checkInWhitelist = (target) => {
    const { whitelist } = this.options;
    return Array.isArray(whitelist)
      && whitelist.some((ele) => ele && typeof ele.contains !== 'undefined' && ele.contains(target));
  }

  handleWheel = (e) => {
    /* istanbul ignore else */
    if (this.checkInWhitelist(e.target)) return;
    e.preventDefault();
  };
